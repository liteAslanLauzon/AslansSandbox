# Snippet: Ring Detection via Image Subtraction, Priority, Extrapolation & Cropping with Aspect-Ratio-Preserving Window Resizing
import cv2
import numpy as np

# Define crop bounds (pixels)
x_min, x_max = 3520, 4400
y_min, y_max = 400, 3585
# Paths for before/after images
path_before = "Projects\\Sac DropV2\\Images\\before.bmp"
path_after = "Projects\\Sac DropV2\\Images\\after.bmp"
...
# Crop images
print(f"Cropping images to x:[{x_min},{x_max}), y:[{y_min},{y_max})...")
...
# Compute window scale for max dimension 600px
max_win = 600
scale = min(max_win / new_w, max_win / new_h)
win_w = int(new_w * scale)
win_h = int(new_h * scale)
print(f"Window size will be: {win_w}x{win_h}")
...
show_window("Detected Contours", initial)
...
show_window("Extrapolated Circles", extrap)
